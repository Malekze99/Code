...
# NEW: Extreme Overbought/Oversold Levels
...
def calculate_features(df: pd.DataFrame, btc_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Overbought/Oversold Filter (Current/Regular)
    ...
    # --- NEW: Extreme Overbought/Oversold Features ---

    # RSI Extreme Overbought/Oversold (Binary)
    ...
    # Stoch RSI Extreme Overbought/Oversold (Binary, using K line)
    ...
    # Strength of Overbought/Oversold (Continuous)
    # This measures how far into the extreme zone the indicator is
...
def prepare_data_for_ml(df: pd.DataFrame, btc_df: pd.DataFrame, symbol: str) -> Optional[Tuple[pd.DataFrame, pd.Series, List[str]]]:
    ...
    # UPDATED: Added new extreme overbought/oversold features
    feature_columns = [
...
def run_training_job():
    ...
    for symbol in symbols_to_train:
        ...
        try:
            ...
            # يمكنك تعديل هذا الشرط بناءً على أداء النموذج بعد إضافة الميزات الجديدة
            if final_model and final_scaler and model_metrics.get('precision_class_1', 0) > 0.35:
                ...
                logger.warning(f"⚠️ [Main] Model for {symbol} is not useful or did not meet minimum performance. Discarding."); failed_models += 1
        except Exception as e:
            logger.critical(f"❌ [Main] A fatal error occurred for {symbol}: {e}", exc_info=True); failed_models += 1
        time.sleep(1) # Small delay to prevent hitting API limits if processing many symbols
...
if __name__ == "__main__":
    ...
    training_thread.daemon = True # Allow the main program to exit even if the thread is running
